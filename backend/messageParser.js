...
class Command {
	...
	static USERSTATE = 101;
	static ROOMSTATE = 102;
 ...
 * 		source: { nick, host },
 * 		parameters: {},
 * 		command: { type, channel, [msg], [isCapRequestEnabled], bot, botParams } | null,
 * 		tags: {} | null,
 * 		parameters
 * 	}
 * }
...
function parseCommand(rawCommandComponent) {
	...
	switch (commandParts[0]) {
		...
		case 'RECONNECT':
			parsedCommand = {
				msg: 'The Twitch IRC server is about to terminate the connection for maintenance.',
				type: Command[commandParts[0]]
			}
			break;
...
function parseParameters(rawParametersComponent, command) {
	...
	if (-1 === paramsIdx) { // no parameters
		command.bot = commandParts.slice(0);
	} else {
		command.bot = commandParts.slice(0, paramsIdx);
		command.botParams = commandParts.slice(paramsIdx).trim();
	}
}

module.exports = {parseMessage, Command};